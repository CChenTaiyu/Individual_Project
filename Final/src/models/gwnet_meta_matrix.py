...
class GWNET(BaseModel):
    ...
    def __init__(self, supports, adp_adj, adp_adj_meta, dst_indices, dropout, residual_channels, dilation_channels, \
        ...
        if adp_adj:
            self.nodevec1 = nn.Parameter(torch.tensor(adp_adj_meta, dtype=torch.float64).float(), requires_grad=True)
            self.nodevec2 = nn.Parameter(torch.tensor(adp_adj_meta, dtype=torch.float64).t().float(),
                                         requires_grad=True)
            self.supports_len += 1
